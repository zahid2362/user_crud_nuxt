    ...
    <form @submit.prevent="submit">
      <div class="grid gap-6 mb-6 md:grid-cols-2">
        ...
        <div class="flex items-center mb-4">
          ...
          >
          <FormErrorMessage
            v-if="userStore?.errors?.is_active && userStore?.errors?.is_active?.length > 0"
...
const fetchData = async () => {
  ...
  userService.show(url,$toast,userStore,router);
};
...
const submit = async () => {
  const url = `${config.public.base_url}/user/${useRoute().params.id}`;
  await userService.update(url,$toast,userStore,router);
};
