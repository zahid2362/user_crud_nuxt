...
class UserService {
    async index(url: string, per_page: Ref<number> , total: Ref<number> , page: Ref<number> , users:Ref<User[]> , $toast: any):Promise<void>{
        try {
            const res:Response = await fetch(`${url}?per_page=${per_page.value}&page=${page.value}`);
            if(res.ok){
